'''
    ...
    3. Botoes (iterativos, servem para trocar a cor das linhas do fractal)
...
class Koch(State):
    ...
    def onEnter(self, game):
        ...
        self.iterations = 0 #numero de passos a serem realizados para gerar o fractal
        ...
        self.recValorN = (110, 120, 80, 60, pyxel.COLOR_WHITE)
        ...
        self.buttonRun = (5, 120, 85, 30, pyxel.COLOR_GREEN)
        self.buttonClean = (5, 160, 85, 30, pyxel.COLOR_YELLOW)
        self.buttonsColor = [ [15, 210, 20, 20, pyxel.COLOR_CYAN, False, True], 
                             [40, 210, 20, 20, pyxel.COLOR_BROWN, False, False],
                             [65, 210, 20, 20, pyxel.COLOR_PURPLE, False, False],
                             [90, 210, 20, 20, pyxel.COLOR_YELLOW, False, False],
                             [115, 210, 20, 20, pyxel.COLOR_GREEN, False, False],
                             [140, 210, 20, 20, pyxel.COLOR_NAVY, False, False],
                             [165, 210, 20, 20, pyxel.COLOR_RED, False, False]]
    ...
    def update(self, game):
        ...
        if (utilities.checkPointOnRec(pyxel.mouse_x, pyxel.mouse_y, self.buttonRun[0], self.buttonRun[1], self.buttonRun[2], self.buttonRun[3])):
            ...
            self.mouseOnMenu = False
            self.mouseOnSair = False
        #checando a posicao do mouse para ver se esta em cima de algum botao de cores
        for botao in self.buttonsColor:
            if (utilities.checkPointOnRec(pyxel.mouse_x, pyxel.mouse_y, botao[0], botao[1], botao[2], botao[3])):
                botao[5] = True
            else:
                botao[5] = False
            ...
            if (utilities.checkPointOnRec(pyxel.mouse_x, pyxel.mouse_y, self.recQuadrantes[0][0], self.recQuadrantes[0][1], self.recQuadrantes[0][2], self.recQuadrantes[0][3])):
                for quadrante in self.recQuadrantes:
                    if (quadrante[4]):
                        ...
                        quadrante[5] = False
            #checando clique em um dos botoes de cor quando o mouse estiver por la, True se ocorreu clique
            for botao in self.buttonsColor:
                if (utilities.checkPointOnRec(pyxel.mouse_x, pyxel.mouse_y, botao[0], botao[1], botao[2], botao[3])):
                    botao[6] = True
                    self.cor_fractal = botao[4]
                else:
                    botao[6] = False
            #checando clique nos outros botoes
            if (self.mouseOnRun):
                self.linhas.clear() #limpando simulacao anterior
                self.iterations += 1 #atualizando iteracoes
                self.generateKoch(self.iterations) #aplicando novo N
            elif (self.mouseOnClear):
                #voltando 1 iteracao do fractal, caso seja >= 1
                if (self.iterations >= 1):
                    self.linhas.clear() #limpando simulacao anterior
                    self.iterations -= 1 #atualizando iteracoes
                    self.generateKoch(self.iterations) #aplicando novo N
                else:
                    #impossivel iteracoes negativas, apenas apagar ultima simulacao
                    self.linhas.clear() #apagando linhas geradas
    ...
    def draw(self, game):
        ...
        pyxel.text(self.buttonRun[0]+11, self.buttonRun[1]+7, "RUN (+1)", pyxel.COLOR_WHITE, self.fonte16) #texto do botao RUN
        ...
        pyxel.text(self.buttonClean[0]+4, self.buttonClean[1]+7, "CLEAR (-1)", pyxel.COLOR_WHITE, self.fonte16) #texto do botao CLEAR
        ...
        pyxel.text(self.recValorN[0]+8, self.recValorN[1]+10, "N = {}".format(str(self.iterations)), pyxel.COLOR_BLACK, self.fonte18) #texto da janela N
        ...
        #desenhando botoes da alteracao de cor
        for botao in self.buttonsColor:
            pyxel.rect(botao[0], botao[1], botao[2], botao[3], botao[4])
            #desenhando efeito de destaque nos botoes de cor ao passar o mouse em cima
            if (botao[5]):
                pyxel.rectb(botao[0], botao[1], botao[2], botao[3], pyxel.COLOR_WHITE)
            #desenhando efeito de destaque fixo no botao de cor escolhido atual
            if (botao[6]):
                pyxel.rectb(botao[0], botao[1], botao[2], botao[3], pyxel.COLOR_ORANGE)
